// Fabric Capability Registry example: compatibility query.
...
int main() {
  ...
  const WorkerBootId boot = *WorkerBootId::Parse("66666666666666666666666666666666");
  if (!registry.RegisterPublisher(*PublisherId::Parse(kPublisher), grant.scope, boot).HasValue()) {
    return 1;
  }
  const EntityId nic = *EntityId::Parse("nic:example-7");
  ...
  speeds.value = *CapabilityValue::NumericSetValue(
      Unit::BitsPerSecond, std::array<std::uint64_t, 2>{100'000'000'000ull, 400'000'000'000ull});
  speeds.provenance = ProvenanceClass::DirectHardwareEnumeration;
  speeds.source_class = EvidenceSourceClass::HardwareEnumeration;

  PublicationRequest request =
      Request(grant, boot, nic, "example-publication-7", EntityGeneration::FromValue(1));
  request.epoch = registry.CurrentEpoch();
  request.claims = {speeds, QuantityClaim("fabric.queue.max_rx_queues", Unit::Count, 128)};
  ...
  Requirement speed_state;
  speed_state.kind = RequirementKind::StateIs;
  speed_state.capability = speeds.capability;
  speed_state.required_state = CapabilityState::Supported;
  Requirement minimum_queues;
  minimum_queues.kind = RequirementKind::Minimum;
  minimum_queues.capability = *CapabilityId::Parse("fabric.queue.max_rx_queues");
  minimum_queues.operand = *CapabilityValue::QuantityValue(64, Unit::Count);
  Requirement speed_member;
  speed_member.kind = RequirementKind::InSet;
  speed_member.capability = speeds.capability;
  speed_member.operand = *CapabilityValue::QuantityValue(400'000'000'000ull, Unit::BitsPerSecond);
  requirement.children = {speed_state, minimum_queues, speed_member};

  const RequirementEvaluation evaluation = registry.Evaluate(nic, requirement);
  std::printf("outcome=%s nodes=%zu\n",
              std::string(RequirementOutcomeName(evaluation.outcome)).c_str(),
              evaluation.nodes_evaluated);
  for (const RequirementNodeResult& node : evaluation.nodes) {
    std::printf("  kind=%s outcome=%s\n", std::string(RequirementKindName(node.kind)).c_str(),
                std::string(RequirementOutcomeName(node.outcome)).c_str());
  }
  std::printf("example=07 result=ok\n");
  return 0;
}
